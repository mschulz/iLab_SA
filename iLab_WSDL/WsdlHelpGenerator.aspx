...
<html>
    ...
    private static string GetEncodedNamespace(string ns) {
        ...
        void WriteMethodMessage(MessageBinding messageBinding, Message message, bool soap12)
        {
            objectId = 0;
    ...
    void Page_Load(object sender, EventArgs e) {
        ...
        //ArrayList methodsTable = new ArrayList();
        ...
        lblMethods.Text = ProcessService(serviceDescriptions);
        ...
        string ProcessService(ServiceDescriptionCollection serviceDescriptions){
            StringBuilder buf = new StringBuilder();
            
            foreach (ServiceDescription description in serviceDescriptions)
            {
                buf.Append("<h2>");
                buf.Append(description.Namespaces + " ");
                buf.AppendLine(description.Name + "</h2>");
                buf.AppendLine(description.Documentation);
                foreach (PortType portType in description.PortTypes)
                {
                    buf.AppendLine("<h3>" +portType.Name + "</h3><br/>");
                    buf.AppendLine(portType.Documentation);
                    buf.AppendLine("<ul>");
                    foreach (Operation operation in portType.Operations)
                    {
                        Object obj = serviceDescriptions.GetMessage(operation.Messages.Input.Message);
                        buf.Append("<li><b>");

                        buf.Append("<br/>");
                        OperationInput opIn = operation.Messages.Input;
                         //buf.Append(opIn.Message);
                         buf.Append(opIn.Name);
                        OperationOutput opOut = operation.Messages.Output;
                        OperationFlow opFlow = operation.Messages.Flow;
                        buf.Append(operation.Name);
                        buf.AppendLine("</b>");
                        buf.AppendLine("<br/>" +operation.Documentation);
...
<%#XmlConvert.DecodeName(serviceDescriptions[0].Services[0].Name)%>
      ...
      <span visible='<%#ShowingMethodList%>' runat="server">
          ...
          <asp:Label ID="lblMethods"  runat="server" />
